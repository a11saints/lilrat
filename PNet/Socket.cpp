...
namespace PNet {
	Socket::Socket(IPVersion ipv, SocketHandle sh) :ipversion{ ipv }, handle{ sh } {
	...
	PResult Socket::RecvAll(void* destination, int numberOfBytes) {
		...
		while (totalBytesReceived < numberOfBytes) {
			...
			PResult result = Recv(bufferOffset, bytesRemaining, totalBytesReceived);
	...
	PResult Socket::SendAll(const void* data, int numberOfBytes) {
		...
		while (totalBytesSent < numberOfBytes) {
			...
			PResult result = Send(bufferOffset, bytesRemaining, totalBytesSent);
			if (result != PResult::P_Success) {
				return PResult::P_GenericError; 
			}
